...
//
// The release handler writes versioned artifacts to a Git state repo as an
// audit trail (one-way write). The Git repo is NEVER read by an external
// reconciler — agent CRDs are managed directly by the API + operator.
type Handler struct {
...
func NewHandler(gitClient *gitstate.Client, agentRepo agent.Repository, k8sReader agent.K8sReader, k8sWriter agent.K8sWriter, bus *eventbus.EventBus, logger *slog.Logger, opts ...HandlerOption) *Handler {
	...
	// AgentCreated: wizard creates agent → release v1
	// AgentConfigChanged: config editor saves → release v2, v3...
	// AgentDeployed is NOT subscribed — deploys use existing releases, they don't create new ones.
...
func (h *Handler) onAgentDeployed(ctx context.Context, event eventbus.Event) {
	...
	ag, _ := event.Payload["agent"].(*agent.Agent)
	if _, err := h.createReleaseForAgent(ctx, agentID, changelog, ag); err != nil {
...
func (h *Handler) onConfigChanged(ctx context.Context, event eventbus.Event) {
	...
	// If the event carries a pre-built agent (config-save without deploy), use it directly
	ag, _ := event.Payload["agent"].(*agent.Agent)
	if _, err := h.createReleaseForAgent(ctx, agentID, changelog, ag); err != nil {
...
// onAgentDeleted writes a tombstone to current.yaml so the agent is archived
// but its release history is preserved for audit. Runs async (best effort).
func (h *Handler) onAgentDeleted(ctx context.Context, event eventbus.Event) {
	slug, _ := event.Payload["agent_slug"].(string)
	agentID, _ := event.Payload["agent_id"].(string)
	namespace, _ := event.Payload["namespace"].(string)
	if slug == "" {
		return
	}
	dir := gitstate.AgentDir(namespace, slug)

	// Audit trail: mark current.yaml as deleted (one-way append-only log).
	path := dir + "/current.yaml"
	...
	_ = h.gitClient.WriteFile(ctx, path, string(yamlBytes), fmt.Sprintf("Archive %s — agent deleted (was v%d)", slug, artifact.Metadata.Version))

	h.logger.Info("agent archived in git state", "slug", slug, "agent_id", agentID)
}
...
func (h *Handler) createReleaseForAgent(ctx context.Context, agentID, changelog string, ag *agent.Agent) (int, error) {
	...
	artifact := gitstate.BuildArtifact(slug, 0, changelog, ag) // version set below
	dir := gitstate.AgentDir(artifact.Deployment.Namespace, slug)

	currentVersion, err := h.highestVersion(ctx, dir)
	if err != nil {
		currentVersion = 0
	}
	nextVersion := currentVersion + 1
	artifact.Metadata.Version = nextVersion
	artifact.Metadata.Previous = nextVersion - 1
	...
	releasePath := fmt.Sprintf("%s/releases/v%d.yaml", dir, nextVersion)
	...
	return nextVersion, h.approveRelease(ctx, agentID, slug, dir, nextVersion)
}

// approveRelease promotes a release to active in the audit log.
// The CRD is managed directly by the API/operator — this only updates Git.
func (h *Handler) approveRelease(ctx context.Context, agentID, slug, dir string, version int) error {
	releasePath := fmt.Sprintf("%s/releases/v%d.yaml", dir, version)
	...
	// Activate: write current.yaml (audit pointer to active version)
	currentPath := dir + "/current.yaml"
...
// rejectRelease marks a release as rejected in the audit log.
// CRD rollback is the responsibility of the API/operator, not the audit trail.
func (h *Handler) rejectRelease(ctx context.Context, agentID, slug, dir string, version int, reason string) error {
	releasePath := fmt.Sprintf("%s/releases/v%d.yaml", dir, version)
	content, err := h.gitClient.ReadFile(ctx, releasePath)
	if err != nil {
		return fmt.Errorf("read release v%d: %w", version, err)
...
func (h *Handler) Create(w http.ResponseWriter, r *http.Request) {
	...
	version, err := h.createReleaseForAgent(r.Context(), ag.ID, req.Changelog, nil)
	if err != nil {
...
func (h *Handler) EvalResult(w http.ResponseWriter, r *http.Request) {
	ag, slug, dir, ok := h.resolveAgent(w, r)
	if !ok {
		return
	}
	...
	if result.Passed {
		if err := h.approveRelease(r.Context(), ag.ID, slug, dir, version); err != nil {
		...
		if err := h.rejectRelease(r.Context(), ag.ID, slug, dir, version, result.Verdict); err != nil {
...
func (h *Handler) List(w http.ResponseWriter, r *http.Request) {
	...
	activeVersion := 0
	currentContent, err := h.gitClient.ReadFile(r.Context(), dir+"/current.yaml")
	if err == nil {
...
func (h *Handler) Get(w http.ResponseWriter, r *http.Request) {
	...
	path := fmt.Sprintf("%s/releases/v%d.yaml", dir, version)
	...
	status := "archived"
	currentContent, readErr := h.gitClient.ReadFile(r.Context(), dir+"/current.yaml")
	if readErr == nil {
		if current, parseErr := gitstate.UnmarshalArtifact(currentContent); parseErr == nil {
...
func (h *Handler) Deploy(w http.ResponseWriter, r *http.Request) {
	...
	// Apply rollback to the cluster directly
...
func (h *Handler) Diff(w http.ResponseWriter, r *http.Request) {
	...
	fromContent, err := h.gitClient.ReadFile(r.Context(), fmt.Sprintf("%s/releases/v%d.yaml", dir, fromVer))
	if err != nil {
		httputil.WriteError(w, http.StatusNotFound, "Not Found", fmt.Sprintf("Release v%d not found", fromVer), r.URL.Path)
		return
	}
	toContent, err := h.gitClient.ReadFile(r.Context(), fmt.Sprintf("%s/releases/v%d.yaml", dir, toVer))
	if err != nil {
...
// resolveAgent fetches the agent by URL param {id} and derives slug + state repo dir.
func (h *Handler) resolveAgent(w http.ResponseWriter, r *http.Request) (*agent.Agent, string, string, bool) {
	if h.agentRepo == nil {
	...
	slug := agentSlug(ag)
	ns := gitstate.DeriveNamespace(ag.TeamID)
	dir := gitstate.AgentDir(ns, slug)
	return ag, slug, dir, true
}

// agentSlug derives a slug from an agent.
func agentSlug(ag *agent.Agent) string {
	if ag.Slug != "" {
		return ag.Slug
	}
...
func (h *Handler) highestVersion(ctx context.Context, agentDir string) (int, error) {
	dir := agentDir + "/releases"
...
// Used by Deploy (rollback) to apply a previous release version directly.
func (h *Handler) applyCRD(ctx context.Context, namespace, agentName string, artifact *gitstate.AgentArtifact) {
	...
	fields := map[string]interface{}{
		"version":  fmt.Sprintf("%d", artifact.Metadata.Version),
