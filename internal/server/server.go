...
func New(cfg config.Config, logger *slog.Logger, agentRepo agent.Repository, kbStore *kb.Store, pool ...*pgxpool.Pool) *Server {
	...
	canaryHandler := agent.NewCanaryHandler(agentRepo, k8sReader, k8sWriter, bus, logger, cfg.MLflowURI)
	// Release handler uses port 8001 (ControlServer) for eval trigger — default is correct
	...
	s := &Server{
		...
		syncHandler:        platform.NewSyncHandler(platformHandler, agentRepo, k8sWriter, logger),
...
func (s *Server) Shutdown(ctx context.Context) error {
	...
	// Close gRPC connections if using gRPC proxy.
